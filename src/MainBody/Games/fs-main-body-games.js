...
function createData(matchday, date, homeTeam, awayTeam, homeGoals, awayGoals) {
  return {matchday, date, homeTeam, awayTeam, homeGoals, awayGoals};
}

function parseRows(result) {
  let rows = [];
  for (let match of result.matches) {
      rows.push(createData(match.matchday, match.utcDate, match.homeTeam.name, match.awayTeam.name, match.score.fullTime.homeTeam, match.score.fullTime.awayTeam));
  }  
  return rows;
}
            ...
            {this.state.isLoaded && matches.map((row)=>
              ...
              (
                <TableRow key={row.name}>
                  <TableCell className="team" align="right">{row.homeTeam}{row.homeUrl}</TableCell>
